...
df<-read.csv('C:\\Users\\yyf\\Desktop\\MCM\\consumption.csv',sep=',')

calEnergies<-data.frame(df %>% dplyr::filter(StateCode=='CA'))
ggplot(data=calEnergies,aes(x=Year,y=introduction)) + 
  ...
  labs(x="Year", y="State", title="California Energy Consumption")